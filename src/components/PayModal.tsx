import React from "react"
import { Box, Dialog } from "@mui/material"
import { BozPay } from "boz.pay.component"

interface PayModalProps {
    open: boolean
    setOpen: (value: boolean) => void
    orderId: string
}

export const PayModal: React.FC<PayModalProps> = ({ open, orderId, setOpen }) => {
    return (
        <Dialog open={open} onClose={() => setOpen(false)} sx={{ width: "100vw", height: "100vh", justifyContent: "center", alignItems: "center" }}>
            <Box sx={{ height: "70vh", flexDirection: "column" }}>
                <BozPay
                    pagseguroToken="5e137c4a-acd6-433a-83a7-736815c6995b0ad8f02a47329494fac489b021d5ab384b54-9b9f-4140-b4cf-4675e700a829"
                    pagseguroTokenSandbox="1BD9D2D2181B4660BAFC9426CA5A63A9"
                    creditCardPublicKey=""
                    storeIdentifier="boz"
                    referenceId={"order1234"}
                    sandbox
                    wrapperSx={{ width: "90vw", alignSelf: "center" }}
                />
            </Box>
        </Dialog>
    )
}
